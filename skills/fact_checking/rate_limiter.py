...
from .models import CircuitBreakerConfig, RateLimitConfig
...
class SourceManager:
    ...
    def register_source(
        self, source_id: str, rate_config: RateLimitConfig, circuit_config: CircuitBreakerConfig
    ):
    ...
    def get_circuit_breaker(self, source_id: str) -> CircuitBreaker | None:
        """Get the circuit breaker for a source, or None if not registered."""
        with self._lock:
            return self._circuit_breakers.get(source_id)

    def can_query(self, source_id: str) -> tuple[bool, str | None]:
        """
