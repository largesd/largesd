...
class AuditLogger:
    ...
    def query_by_claim_hash(self, claim_hash: str, limit: int = 100) -> list[dict[str, Any]]:
    ...
    def query_by_request_id(self, request_id: str) -> dict[str, Any] | None:
        """Query audit log by request ID"""
        with self._connect() as conn:
            conn.row_factory = sqlite3.Row
            cursor = conn.execute("SELECT * FROM audit_log WHERE request_id = ?", (request_id,))
            row = cursor.fetchone()
            return dict(row) if row else None

    def query_by_post_id(self, post_id: str, limit: int = 100) -> list[dict[str, Any]]:
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        with self._connect() as conn:
            ...
            cursor = conn.execute("SELECT COUNT(DISTINCT claim_hash) FROM audit_log")
