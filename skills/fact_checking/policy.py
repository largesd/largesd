...
def _is_fresh(result: SourceResult, max_age: timedelta | None) -> bool:
...
def apply_policy(
    ...
    if (
        from_ground_truth
        and policy.ground_truth_sufficient
        and (tier1_confirms or tier1_contradicts)
    ):
    ...
    if policy.strict_mode or not policy.tier2_can_resolve:
        return (
            False,
            "INSUFFICIENT",
            "No Tier-1 resolution path succeeded and policy prohibits lower-tier resolution.",
        )
