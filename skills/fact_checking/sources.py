        ...
        for allowlist in self._allowlists.values():
            for source in allowlist.approved_sources:
                self._source_manager.register_source(
                    source.source_id, source.rate_limit, allowlist.circuit_breaker_config
                )

    def get_allowlist(self, version: str) -> AllowlistVersion | None:
        """Get allowlist by version"""
        return self._allowlists.get(version)

    def get_source(self, source_id: str) -> ApprovedSource | None:
...
class EvidenceRetriever:
    ...
    def retrieve_evidence(
        self, normalized_claim: str, claim_hash: str, allowlist_version: str
        ...
        for source in allowlist.approved_sources:
            ...
            try:
                ...
                source_evidence = self._query_source(source, normalized_claim, claim_hash)
        ...
        selected_evidence = self._select_evidence(evidence_candidates, allowlist.evidence_keep_n)

        return selected_evidence, sources_considered

    def _query_source(
        self, source: ApprovedSource, normalized_claim: str, claim_hash: str
    ) -> list[EvidenceRecord]:
        """
    ...
    def _select_evidence(
        self, candidates: list[EvidenceRecord], keep_n: int
    ) -> list[EvidenceRecord]:
        """
