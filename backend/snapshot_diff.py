...
class FactChange:
    ...
    fact_text: str
    side: str
    p_true_old: float | None = None
    p_true_new: float | None = None
    # LSD_FactCheck_v1_5_1 ternary fields
    v15_status_old: str | None = None
    v15_status_new: str | None = None
    v15_insufficiency_reason_old: str | None = None
    v15_insufficiency_reason_new: str | None = None
...
class ScoreChange:
    ...
    topic_id: str | None  # None for overall
    side: str | None  # None for overall
            ...
            "posts": [
                {
                    ...
                    "side": p.side,
                    "topic_id": p.topic_id,
                    "facts_preview": p.facts_preview[:100] + "..."
                    if len(p.facts_preview) > 100
                    else p.facts_preview,
                    "inference_preview": p.inference_preview[:100] + "..."
                    if len(p.inference_preview) > 100
                    else p.inference_preview,
                }
                for p in self.posts
            ],
            "topics": [
            ...
            "facts": [
                {
                    ...
                    "fact_id": f.fact_id,
                    "fact_text": f.fact_text[:100] + "..."
                    if len(f.fact_text) > 100
                    else f.fact_text,
                    "side": f.side,
            ...
            "arguments": [
                {
                    ...
                    "arg_id": a.arg_id,
                    "inference_text": a.inference_text[:100] + "..."
                    if len(a.inference_text) > 100
                    else a.inference_text,
                    "side": a.side,
                }
...
class SnapshotDiffEngine:
    ...
    def diff_snapshots(self, snapshot_id_old: str, snapshot_id_new: str) -> SnapshotDiff:
        """
    ...
    def _diff_facts(self, snap_old: dict, snap_new: dict) -> list[FactChange]:
        ...
        json.loads(snap_old.get("topic_scores", "{}") or "{}")
        json.loads(snap_new.get("topic_scores", "{}") or "{}")
        ...
        for fact_id, fact in new_facts.items():
            if fact_id not in old_facts:
                ...
                reason_changed = old_fact.get("v15_insufficiency_reason") != fact.get(
                    "v15_insufficiency_reason"
                )
