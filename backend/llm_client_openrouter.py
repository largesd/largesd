...
class OpenRouterProvider(LLMProvider):
    ...
    def generate(self, prompt: str, temperature: float = 0.7, max_tokens: int = 500) -> LLMResponse:
        ...
        try:
            ...
            allow_fallback = os.getenv("ALLOW_MOCK_FALLBACK", "false").lower() in (
                "1",
                "true",
                "yes",
            )
...
class MultiModelJudgeProvider(LLMProvider):
    ...
    def __init__(
        ...
        self.judges = [
            OpenRouterProvider(
                api_key=self.api_key, model=model, site_url=self.site_url, site_name=self.site_name
            )
    ...
    def get_judge_info(self) -> list:
        ...
        return [
            {"judge_id": i, "model": model, "provider": "openrouter"}
