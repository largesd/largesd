...
from backend.models import CanonicalArgument, CanonicalFact, Side, Topic, TopicSideScores
...
class ScoringEngine:
    ...
    def compute_reasoning_strength(
        self, arguments: list[CanonicalArgument], side: Side
    ) -> tuple[float, float]:
        """
    ...
    def compute_quality(self, factuality: float, reasoning: float, coverage: float) -> float:
        """
    ...
    def compute_topic_relevance(
        self, topics: list[Topic], topic_content_mass: dict[str, int]
    ) -> dict[str, float]:
        """
        ...
        return {topic_id: mass / total_mass for topic_id, mass in topic_content_mass.items()}
    ...
    def compute_debate_scores(
        ...
        for topic in topics:
            ...
            # Compute FOR scores
            f_for = self.compute_factuality(
                for_facts[: len(for_facts) // 2 + 1] if for_facts else []
            )
            ...
            f_against = self.compute_factuality(
                against_facts[len(against_facts) // 2 :] if against_facts else []
            )
            reason_against, reason_iqr_against = self.compute_reasoning_strength(
                against_args, Side.AGAINST
            )
    ...
    def run_replicates(
        self, topics, topic_facts, topic_arguments, topic_content_mass
        ...
        for _ in range(self.num_replicates):
            ...
            for tid, args in topic_arguments.items():
                noisy_args[tid] = []
                for a in args:
                    noisy_reasoning = np.clip(a.reasoning_score + np.random.normal(0, 0.08), 0, 1)
