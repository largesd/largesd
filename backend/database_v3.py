...
class DatabaseV3(DebateDatabase):
    ...
    def set_user_preference(self, user_id: str, key: str, value: str):
        ...
        cursor.execute(
            ...
            VALUES (?, ?, ?, ?, ?)
        """,
            (str(uuid.uuid4()), user_id, key, value, datetime.now().isoformat()),
        )
        conn.commit()
        conn.close()
    ...
    def get_debates_by_user(self, user_id: str) -> list[dict[str, Any]]:
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT * FROM debates WHERE user_id = ? ORDER BY created_at DESC", (user_id,)
        )
        rows = cursor.fetchall()
    ...
    def get_topic(self, topic_id: str, debate_id: str) -> dict[str, Any] | None:
        """Get specific topic by ID and debate"""
        conn = self._get_connection()
        cursor = conn.cursor()
        cursor.execute(
            "SELECT * FROM topics WHERE topic_id = ? AND debate_id = ?", (topic_id, debate_id)
        )
        row = cursor.fetchone()
        conn.close()
    ...
    def _is_legacy_sha256_hash(self, password_hash: str) -> bool:
        return len(password_hash) == 64 and all(
            ch in "0123456789abcdefABCDEF" for ch in password_hash
        )
