...
class ExtractedFact:
    ...
    p_true: float = 0.5
    fact_check_job_id: str | None = None  # For async fact checking
    ...
    v15_status: str | None = None  # SUPPORTED, REFUTED, INSUFFICIENT
    v15_p: float = 0.5
    v15_insufficiency_reason: str | None = None
    v15_human_review_flags: list[str] = field(default_factory=list)
    v15_best_evidence_tier: int | None = None
...
class ExtractedArgument:
    ...
    inference_span: ExtractedSpan | None = None
...
class ExtractionEngine:
    ...
    ) -> tuple[list[ExtractedSpan], ExtractedSpan]:
        """
    ...
    def extract_facts_from_spans(
        self, fact_spans: list[ExtractedSpan], topic_id: str, side: str, post_id: str | None = None
    ) -> list[ExtractedFact]:
        """
        ...
        for i, span in enumerate(fact_spans):
            ...
            if self.fact_checker and fact_type == "empirical":
                try:
                    ...
                    if hasattr(self.fact_checker, "check_fact_async") and getattr(
                        self.fact_checker, "_async_enabled", False
                    ):
                        job = self.fact_checker.check_fact_async(
                            claim_text=span.span_text, request_context=request_context
                        )
                        ...
                        fact.operationalization = (
                            result.operationalization or fact.operationalization
                        )
    ...
    def create_argument_units(
        self,
        fact_spans: list[ExtractedSpan],
        inference_span: ExtractedSpan,
        ...
        if inference_span:
            ...
            raise ValueError("AU_INFERENCE has no provenance span. MSD §6.1 hard rule violated.")
    ...
    def canonicalize_facts(
        self, facts: list[ExtractedFact], topic_scope: str
    ) -> list[CanonicalFact]:
        """
        ...
        for i, cluster in enumerate(result.get("clusters", [])):
            ...
            fact_type = (
                "normative"
                if "normative" in fact_types
                else "definitional"
                if "definitional" in fact_types
                else "empirical"
            )
            ...
            cluster_v15_p = round(sum(mf.v15_p for mf in member_facts) / len(member_facts), 2)
            v15_reasons = [
                mf.v15_insufficiency_reason for mf in member_facts if mf.v15_insufficiency_reason
            ]
            cluster_v15_reason = (
                v15_reasons[0]
                if v15_reasons
                else ("mixed_cluster_evidence" if len(set(v15_statuses)) > 1 else None)
            )
            cluster_v15_flags = list(
                set(flag for mf in member_facts for flag in mf.v15_human_review_flags)
            )
            v15_tiers = [
                mf.v15_best_evidence_tier
                for mf in member_facts
                if mf.v15_best_evidence_tier is not None
            ]
            ...
            cf = CanonicalFact(
                ...
                fact_type=fact_type,
                normative_provenance="; ".join(
                    mf.normative_provenance for mf in member_facts if mf.normative_provenance
                ),
                operationalization=member_facts[0].operationalization,
                evidence_tier_counts=tier_counts,
                fact_check_diagnostics=member_facts[0].fact_check_diagnostics
                if member_facts
                else {},
    ...
    def canonicalize_arguments(
        self,
        argument_units: list[ExtractedArgument],
        canonical_facts: list[CanonicalFact],
        topic_scope: str,
    ) -> list[CanonicalArgument]:
        """
        ...
        for au in argument_units:
            ...
            for fact_text in au.au_facts:
                for cf in canonical_facts:
                    if fact_text == cf.canon_fact_text or fact_text in [
                        f.fact_text
                        for f in [
                            ExtractedFact(
                                fact_id="", fact_text=fact_text, topic_id="", side="", p_true=0.5
                            )
                        ]
                    ]:
    ...
    def compute_extraction_stability(
        self, posts: list[dict], topic_scope: str, num_runs: int = 2
    ) -> dict:
        """
        ...
        for _ in range(num_runs):
            ...
            for post in posts:
                ...
                facts = self.extract_facts_from_spans(
                    fact_spans, post.get("topic_id", "unknown"), post["side"], post.get("post_id")
                )
                run_facts.extend(facts)

                # Create argument units
                aus = self.create_argument_units(
                    fact_spans, inf_span, facts, post.get("topic_id", "unknown"), post["side"]
                )
                run_args.extend(aus)
        ...
        mismatches = self._find_mismatches(
            all_run_facts[0], all_run_facts[1] if len(all_run_facts) > 1 else []
        )
    ...
    def _find_mismatches(
        self, facts1: list[CanonicalFact], facts2: list[CanonicalFact]
    ) -> list[dict]:
