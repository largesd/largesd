...
def _topic_meta(topics: list[Any]) -> list[dict[str, Any]]:
    return [
        {
            "topic_id": t.topic_id,
            "name": t.name,
...
def _topic_meta_parents(topics: list[Any]) -> list[dict[str, Any]]:
    return [
        {
            "topic_id": t.topic_id,
            "name": t.name,
...
def audit_stage(ctx: PipelineContext) -> PipelineContext:
    ...
    # Decision dossier
    dd = engine._build_decision_dossier(
        topics, cfacts, cargs, sfacts, sargs, scores.get("topic_scores", {})
    )
    dd["counterfactuals"] = ctx.counterfactuals or []
    dd["unselected_tail_summary"] = unselected_tail_summary(cfacts, cargs, sfacts, sargs)
    dd["insufficiency_sensitivity"] = insufficiency_sensitivity(
        scores.get("topic_scores", {}), scores.get("margin_d", 0.0)
    )
    dd["formula_metadata"] = formula_registry()
    dd["normative_symmetry"] = ctx.symmetry_result or {}

    # Core audits
    stability = engine.extraction_engine.compute_extraction_stability(
        allowed_posts, topics[0].scope if topics else ""
    )
    symmetry = engine.scoring_engine.run_side_label_symmetry_audit(
        _topic_meta(topics), cfacts, cargs, mass, side_order=side_order, frame_context=frame_context
    )
    relevance = engine.scoring_engine.compute_relevance_sensitivity(
        _topic_meta(topics), cfacts, cargs, mass, side_order=side_order, frame_context=frame_context
    )
    ...
    p2r = (
        engine._remap_to_replicate(topics, rep_topics, mass, sfacts, sargs)[3] if rep_topics else {}
    )
    rep_md = next(
        (
            {"margin_d": getattr(r, "margin_d", 0.0)}
            for r in (ctx.replicates or [])
            if getattr(r, "metadata", {}).get("replicate_type") == "merge_variant"
        ),
        {},
    )
    merge = merge_sensitivity(
        _topic_meta_parents(topics),
        _topic_meta_parents(rep_topics),
        mass,
        scores.get("margin_d", 0.0),
        rep_md.get("margin_d", 0.0),
        p2r,
    )

    ev = evaluator_variance_from_scores(
        scores.get("topic_scores", {}), scores.get("overall_scores", {})
    )
    ...
    frame_sens = {
        ...
        "max_delta_d": 0.0,
        "interpretation": "inactive_single_frame"
    ...
    # Manifests / hashes
    replay = engine._build_replay_manifest(
        ctx.debate_id, selection_seed, allowed_posts, blocked_posts, topics, side_order
    )
    ...
    ob = {
        "topics": [
            {"topic_id": t.topic_id, "name": t.name, "relevance": t.relevance} for t in topics
        ],
        "overall_scores": scores.get("overall_scores", {}),
        "verdict": (ctx.verdict_result or {}).get("verdict"),
    ...
    prov = {
        "provider": rt.get("provider", usage.get("provider", "mock")),
        "configured_model": rt.get("configured_model", "mock"),
        "actual_model": mdl,
        "num_judges": rt.get("num_judges", engine.num_judges),
    ...
    ctx.audit_records = {
        ...
        "side_label_symmetry": symmetry,
        "normative_symmetry": ctx.symmetry_result or {},
        "relevance_sensitivity": relevance,
        "topic_drift": ctx.drift_report,
        "selection_transparency": ctx.selection_diagnostics or {},
        "verdict_replicates": ctx.verdict_result or {},
        "decision_dossier": dd,
        "evaluator_variance": ev,
        "topic_diagnostics": tdiag,
        "topic_merge_sensitivity": merge,
        "participation_concentration": part,
