...
from backend.lsd_v1_2 import compute_borderline_rate, build_suppression_policy, SUPPRESSION_K


def _make_span(s: dict) -> ExtractedSpan:
    return ExtractedSpan(
        span_id=s["span_id"], post_id=s["post_id"],
        start_offset=s["start_offset"], end_offset=s["end_offset"],
        span_text=s["span_text"], topic_id=s.get("topic_id"),
        side=s["side"], span_type=s["span_type"],
    )
...
def extract_stage(ctx: PipelineContext) -> PipelineContext:
    ...
    for topic in topics:
        engine.db.save_topic({
            "topic_id": topic.topic_id, "debate_id": debate_id, "frame_id": frame_id,
            "name": topic.name, "scope": topic.scope, "relevance": topic.relevance,
            "drift_score": topic.drift_score, "coherence": topic.coherence,
            "distinctness": topic.distinctness, "parent_topic_ids": topic.parent_topic_ids,
            "operation": topic.operation, "summary_for": topic.summary_for,
    ...
    for topic in topics:
        tid = topic.topic_id
        topic_posts = [p for p in allowed_posts if p.get("topic_id") == tid or p["post_id"] in post_assignments.get(tid, [])]
        ...
        for post in topic_posts:
            ...
            facts = engine.extraction_engine.extract_facts_from_spans(fact_spans, tid, post["side"], post_id=post["post_id"])
            all_facts.extend(facts)
            if inference_span:
                all_args.extend(engine.extraction_engine.create_argument_units(fact_spans, inference_span, facts, tid, post["side"]))

        extracted[tid] = {"topic": topic, "topic_posts": topic_posts, "spans": all_spans, "facts": all_facts, "args": all_args}
