...
@optional_auth
def get_verdict() -> Any:
    """Get verdict for the current debate.
    ---
    tags:
      - dossier
    security:
    ...
    for topic in topics:
        ...
        contribution = topic["relevance"] * (
            for_scores.get("quality", 0) - against_scores.get("quality", 0)
        )
    ...
    return jsonify(
        {
            ...
            "replicate_composition_metadata": {
                "judge_count": int(os.getenv("NUM_JUDGES", "5")),
                "replicate_count": verdict_replicates.get("replicate_composition", {}).get(
                    "replicate_count", len(verdict_replicates.get("d_distribution", []))
                ),
                "extraction_reruns": 2,
                "bootstrap_samples": verdict_replicates.get("replicate_composition", {}).get(
                    "bootstrap_samples", len(verdict_replicates.get("d_distribution", []))
                ),
                "structural_replicate_count": verdict_replicates.get(
                    "replicate_composition", {}
                ).get("structural_replicate_count", 0),
...
@optional_auth
def get_audits() -> Any:
    """Get audits for the current debate.
    ---
    tags:
      - dossier
    security:
    ...
    return jsonify(
        {
            "snapshot_id": snapshot["snapshot_id"],
            "audit_schema_version": __import__(
                "backend.lsd_v1_2", fromlist=["AUDIT_SCHEMA_VERSION"]
            ).AUDIT_SCHEMA_VERSION,
