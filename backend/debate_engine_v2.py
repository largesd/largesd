    ...
    def __init__(self, db_path: str = "data/debate_system.db",
        ...
        if self._fact_check_mode == "PERFECT":
            self.fact_checker = FactCheckingSkill(
                ...
                enable_async=False,
                connectors=[
                    # PERFECT is strict production mode for the narrow v1
                    # supported family only. Keep the connector set limited to
                    # the Tier-1 structured path.
                    WikidataConnector(),
                ],
            )
        elif self._fact_check_mode == "PERFECT_CHECKER":
            self.fact_checker = FactCheckingSkill(
                mode=self._fact_check_mode,
                allowlist_version="v1",
                enable_async=False,
                connectors=[
                    # PERFECT_CHECKER is deterministic fixture/test mode, so it
                    # can keep controlled lower-tier integration stubs alongside
                    # the narrow Tier-1 connector.
                    WikidataConnector(),
