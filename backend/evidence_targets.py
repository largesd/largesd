...
class EvidenceTargetAnalyzer:
    ...
    def analyze_evidence_targets(
        self, debate_id: str, snapshot_id: str | None = None
    ) -> VerdictSensitivity:
        """
        ...
        for arg in arguments:
            ...
            if supporting:
                avg_decisiveness = sum(fact_decisiveness.get(fid, 0) for fid in supporting) / len(
                    supporting
                )
            else:
        ...
        for arg in arguments:
            ...
            if lev_score > 0.3:
                ...
                decisive_facts = [f for f in facts if f.get("canon_fact_id", "") in supporting]
                ...
                target = EvidenceTarget(
                    ...
                    update_trigger="If key supporting facts of this argument are refuted, "
                    "the argument's leverage would decrease significantly.",
        ...
        for ts in topic_scores:
            ...
            if 0.3 < quality < 0.6:
                target = EvidenceTarget(
                    ...
                    description="Topic-side with moderate quality that could swing",
                    side=side,
                    ...
                    evidence_type="mixed",
                    update_trigger="If evidence quality in this topic improves significantly, "
                    "the overall verdict could shift.",
                )
                all_targets.append(target)
    ...
    def _load_latest_snapshot(self, debate_id: str) -> dict | None:
    ...
    def _load_topic_scores(self, snapshot_id: str) -> list[dict]:
        ...
        # Convert to list
        result = []
        for _key, scores in topic_scores.items():
    ...
    def _suggest_evidence_for_fact(self, fact: dict) -> str:
        ...
        if any(
            word in fact_text for word in ["law", "legal", "regulation", "court", "jurisdiction"]
        ):
    ...
    def _suggest_evidence_for_argument(self, argument: dict, decisive_facts: list[dict]) -> str:
        """Suggest evidence needed to support/refute an argument"""
        argument.get("inference_text", "").lower()

        if decisive_facts:
            # Argument depends on facts
            fact_summary = "; ".join(
                [f.get("canon_fact_text", "")[:50] + "..." for f in decisive_facts[:2]]
            )
    ...
    def _generate_update_trigger(self, fact: dict, dependent_args: list[dict]) -> str:
        ...
        if status == "REFUTED":
            return (
                f"If FACT {fact_id[:20]}... is confirmed (SUPPORTED) with strong evidence, "
                f"it would strengthen {arg_count} dependent argument(s), "
                f"potentially shifting the topic quality score."
            )
        elif status == "SUPPORTED":
            return (
                f"If FACT {fact_id[:20]}... is refuted (REFUTED) with strong evidence, "
                f"it would weaken {arg_count} dependent argument(s), "
                f"potentially shifting the topic quality score."
            )
        else:
            return (
                f"If FACT {fact_id[:20]}... is resolved to SUPPORTED or REFUTED with strong evidence, "
                f"it would affect {arg_count} dependent argument(s), "
                f"potentially shifting the topic quality score."
            )

    def _generate_summary(
        self, verdict: str, confidence: float, targets: list[EvidenceTarget]
    ) -> str:
        """Generate human-readable summary"""
        if not targets:
            return (
                f"The current verdict ({verdict}) appears stable with {confidence:.0%} confidence. "
        ...
        summary = (
            f"Current verdict: {verdict} with {confidence:.0%} confidence. "
            f"The outcome is most sensitive to: {top_target.title}. "
        )

        if top_target.target_type == "fact":
            summary += (
                "If this fact were resolved with decisive evidence, " "the verdict could change."
            )
        elif top_target.target_type == "argument":
            summary += (
                "If the key supporting facts of this argument are challenged, "
...
def get_evidence_targets(debate_engine, debate_id: str, snapshot_id: str | None = None) -> dict:
    """
