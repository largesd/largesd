...
from backend.modulation import ModulationOutcome as ModOutcome
...
def test_modulation_system():
    ...
    print(
        f"✓ Modulation audit info: {audit_info['template_name']} v{audit_info['template_version']}"
    )
...
def test_debate_proposal_requirements():
    ...
    assert (
        proposal["resolution"] == proposal["motion"]
    ), "Motion should hydrate the legacy resolution field"
    ...
    assert proposal["active_frame"][
        "evaluation_criteria"
    ], "Structured frame should include evaluation criteria"
...
def test_frame_versioning_and_multiside_snapshot():
    ...
    try:
        ...
        debate = engine.create_debate(
            {
                ...
                "frame": {
                    ...
                    "sides": [
                        ...
                        {
                            "label": "REGULATE",
                            "description": "Allow deployment under tighter regulation.",
                        },
                        {
                            "label": "OPEN",
                            "description": "Allow broad deployment with minimal new restrictions.",
                        },
                    ],
                    "evaluation_criteria": [
                        "Risk reduction",
                    ...
                    "definitions": [
                        {
                            "term": "Frontier AI",
                            "definition": "Models at the capability frontier for general-purpose use.",
                        }
                    ],
                    "scope_constraints": [
        ...
        updated = engine.create_frame_version(
            debate["debate_id"],
            {
                ...
                "frame": {
                    ...
                    "sides": [
                        {
                            "label": "PAUSE",
                            "description": "Pause new deployment until evaluations improve.",
                        },
                        {
                            "label": "REGULATE",
                            "description": "Permit deployment with stronger licensing and audits.",
                        },
                        {
                            "label": "OPEN",
                            "description": "Permit broad deployment with minimal new constraints.",
                        },
                    ],
                    "evaluation_criteria": [
                        "Risk reduction",
                    ...
                    "definitions": [
                        {
                            "term": "Deployment",
                            "definition": "Releasing a frontier model for public or enterprise use.",
                        }
                    ],
                    "scope_constraints": [
        ...
        assert updated["active_frame"][
            "supersedes_frame_id"
        ], "Frame versions should preserve lineage"
        ...
        assert (
            snapshot["frame_id"] == updated["active_frame_id"]
        ), "Snapshot should bind to the active frame"
        assert snapshot["side_order"] == [
            "PAUSE",
            "REGULATE",
            "OPEN",
        ], "Snapshot should preserve frame side order"
...
def test_fact_canonicalization():
    ...
    from backend.extraction import CanonicalFact, ExtractedFact, ExtractedSpan, ExtractionEngine
    ...
    span1 = ExtractedSpan(
        "s1", "post1", 0, 50, "Renewable energy creates jobs.", "t1", "FOR", "fact"
    )
...
def test_scoring_formulas():
    ...
    assert (
        abs(factuality - expected) < 0.01
    ), f"Factuality should be mean of p_true: {factuality} vs {expected}"
...
def test_verdict_computation():
    ...
    for_replicates = [MockReplicate(0.8, 0.4, 0.4, {}) for _ in range(100)]
    ...
    against_replicates = [MockReplicate(0.4, 0.8, -0.4, {}) for _ in range(100)]
    ...
    uncertain_replicates = [MockReplicate(0.6, 0.55, 0.05, {}) for _ in range(50)] + [
    ...
    print("✓ Uncertain case returns NO VERDICT")
...
def test_full_pipeline():
    ...
    try:
        ...
        # 4. Generate snapshot
        snapshot = engine.generate_snapshot(debate_id=debate["debate_id"], trigger_type="manual")
        ...
        print(
            f"✓ Generated snapshot: verdict={snapshot['verdict']}, confidence={snapshot['confidence']}"
        )
...
def test_v2_uses_skill_fact_checker():
    ...
    try:
        ...
        assert (
            engine.fact_checker.__class__.__name__ == "V15FactCheckingSkill"
        ), f"Expected V15FactCheckingSkill, got {type(engine.fact_checker)}"
        assert (
            engine.extraction_engine.fact_checker is engine.fact_checker
        ...
        assert (
            "cache" in stats and "audit" in stats and "queue" in stats
        ), "Stats should expose fact-check internals"
...
def test_v2_resolves_pending_fact_checks_before_scoring():
    ...
    try:
        ...
        assert (
            resolved[0].fact_check_status == "completed"
        ), f"Expected completed, got {resolved[0].fact_check_status}"
        assert (
            abs(resolved[0].p_true - 0.82) < 1e-9
        ), f"Expected resolved p_true, got {resolved[0].p_true}"
        ...
        assert (
            timed_out[0].fact_check_status == "failed"
...
def test_relevance_sensitivity():
    ...
    audit = engine.compute_relevance_sensitivity(
        topics, topic_facts, topic_arguments, topic_content_mass, num_perturbations=20
    )
    ...
    print(
        f"✓ Relevance sensitivity: D_mean={audit['d_mean']}, stability={audit['stability_ratio']}"
    )
...
def test_merge_variant_deterministic_ids():
    ...
    assert ids1 != ids3 or len(ids1 & ids3) < len(
        ids1
...
def test_merge_sensitivity_ancestry_stability():
    ...
    assert (
        result["mapping_stability"] == expected_stability
...
def test_identity_blindness():
    ...
    from backend.models import CanonicalArgument, Post, Span
...
def test_admin_template_persistence_and_engine_sync():
    ...
    try:
        ...
        activated = db.activate_moderation_template(
            draft["template_record_id"], author_user_id="admin_test"
        )
        assert activated is not None, "Activating a draft should return the activated row"
        assert (
            activated["is_current"] is True
        ), "Activated template should become current pointer target"
        ...
        assert (
            modulation_info["template_version"] == "9.9.1"
        ), "Engine should reflect active moderation template version from DB"
        assert (
            modulation_info["template_name"] == "Minimal Acceptance Template"
...
def test_api_auth_session_and_admin_access_consistency():
    ...
    try:
        ...
        try:
            ...
            if exc.name == "jwt":
                print(
                    "⚠ Skipping API auth/session consistency test: PyJWT not available in this interpreter."
                )
                return
            raise
        ...
        assert (
            unauthorized_create.status_code == 403
        ), "Unauthenticated debate creation without CSRF should be rejected"
        ...
        registration = client.post(
            "/api/auth/register", headers={"X-CSRF-Token": csrf_token}, json=registration_payload
        )
        assert (
            registration.status_code == 201
        ), f"Registration failed: {registration.get_data(as_text=True)}"
        ...
        ), f"Authenticated debate creation failed: {create.get_data(as_text=True)}"
        ...
        assert (
            active_payload.get("debate_id") == debate_id
        ), "Active debate should persist per authenticated user"

        anonymous_view = client.get("/api/debate")
        assert anonymous_view.status_code == 200
        assert (
            anonymous_view.get_json().get("has_debate") is False
        ...
        ), "Restricted admin mode should block non-allowlisted users"
        ...
        assert (
            admin_allowed.status_code == 200
        ), "Allowlisted restricted admin user should gain access"
        assert admin_allowed.get_json().get(
            "template"
        ), "Admin response should include active template"

        logout = client.post("/api/auth/logout", headers=auth_headers)
        assert (
            logout.status_code == 200
        ), "Logout endpoint should succeed for authenticated sessions"
        ...
        assert (
            expired_check.get_json().get("code") == "AUTH_INVALID"
        ), "Expired token should map to AUTH_INVALID"
...
def test_legacy_password_hashes_authenticate_and_migrate():
    ...
    try:
        ...
        db.save_user(
            {
                ...
                "password_hash": hashlib.sha256(b"legacy-pass-456").hexdigest(),
        ...
        assert migrated_scrypt["password_hash"].startswith(
            "$2"
        ), "Legacy scrypt hash should be upgraded to bcrypt"
        assert migrated_sha256["password_hash"].startswith(
            "$2"
        ), "Legacy SHA-256 hash should be upgraded to bcrypt"
...
def test_rate_limiter_exempts_navigation_and_read_only_requests():
    ...
    try:
        ...
        ), "Mutating API requests should still hit the default rate limit"
...
def test_debate_proposal_lifecycle():
    ...
    try:
        ...
        assert any(
            p["proposal_id"] == proposal_id for p in user_proposals
        ), "Proposal should be in user's list"

        pending_queue = db.get_debate_proposals_by_status("pending")
        assert any(
            p["proposal_id"] == proposal_id for p in pending_queue
        ), "Proposal should be in pending queue"
        ...
        assert (
            updated["accepted_debate_id"] == debate["debate_id"]
        ), "Accepted debate ID should be stored"
...
def test_accepting_proposal_activates_debate_for_proposer():
    ...
    try:
        ...
        assert (
            proposer_active_payload.get("debate_id") == debate_id
        ...
        assert (
            debate_id in debate_ids
        ), "Accepted debate should appear in the proposer's debate list"
...
def test_snapshot_append_only_and_integrity_fields():
    ...
    try:
        ...
        engine.submit_post(debate["debate_id"], "FOR", "t1", "Fact one.", "Therefore one.")
        engine.submit_post(debate["debate_id"], "AGAINST", "t1", "Fact two.", "Therefore two.")

        snapshot = engine.generate_snapshot(debate["debate_id"], trigger_type="manual")

        # Verify integrity fields present
        assert (
            "replay_manifest_json" in snapshot or "replay_manifest" in snapshot
        ), "Snapshot should include replay manifest"
        ...
        assert snapshot.get("recipe_versions_json") or snapshot.get(
            "recipe_versions"
        ), "Snapshot should include recipe_versions"
...
def test_published_bundle_uses_engine_for_diff_and_targets():
    ...
    try:
        ...
        engine.submit_post(
            debate_id, "FOR", None, "Renewables create jobs.", "Therefore subsidize."
        )
        engine.submit_post(
            debate_id, "AGAINST", None, "Subsidies distort markets.", "Therefore do not subsidize."
        )
        ...
        engine.submit_post(
            ...
            "FOR",
            None,
            "Solar costs have dropped 80%.",
            "Therefore subsidies are efficient.",
        )
        engine.generate_snapshot(debate_id, trigger_type="activity")
        ...
        assert (
            "high_impact_targets" in targets
        ), "Engine targets should include 'high_impact_targets'"
        assert (
            "medium_impact_targets" in targets
        ), "Engine targets should include 'medium_impact_targets'"
        assert (
            "margin_needed_for_flip" in targets
        ), "Engine targets should include 'margin_needed_for_flip'"
        ...
        assert (
            "verdict_sensitivity" not in targets
        ), "Targets should remain flat, not nested under 'verdict_sensitivity'"
...
def run_all_tests():
    ...
    tests = [
        ...
        (
            "DebateFrame Versioning & Multi-Side Snapshot",
            test_frame_versioning_and_multiside_snapshot,
        ),
        ...
        (
            "Admin Template Persistence + Engine Sync",
            test_admin_template_persistence_and_engine_sync,
        ),
        ("API Auth + Session Consistency", test_api_auth_session_and_admin_access_consistency),
        (
            "Legacy Password Hash Compatibility",
            test_legacy_password_hashes_authenticate_and_migrate,
        ),
...
def test_sufficiency_checks_return_insufficient_data():
    ...
    try:
        ...
        assert (
            vdata.get("verdict") == "INSUFFICIENT_DATA"
        ), f"Expected INSUFFICIENT_DATA, got {vdata.get('verdict')}"
        ...
        dossier_resp = client.get(
            f"/api/debate/decision-dossier?debate_id={debate_id}", headers=headers
        )
