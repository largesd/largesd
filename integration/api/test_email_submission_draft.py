...
class TestEmailSubmissionDraftBody:
    ...
    def test_logged_in_receives_v3_body_and_mailto(
    ...
    def test_response_payload_hash_matches_token_claim(
        ...
        token_line = [
            l for l in data["body"].split("\n") if l.startswith("Auth-Token:")
        ][0]
...
class TestEmailSubmissionDraftValidation:
    ...
    def test_empty_inference_returns_validation_error(self, client, auth_headers, draft_debate):
        ...
        assert resp.status_code == 400
        data = resp.get_json()
        assert data.get("code") == "VALIDATION_ERROR" or "inference" in data.get("error", "").lower()
...
class TestEmailSubmissionDraftErrors:
    ...
    def test_cache_only_debate_id_returns_not_available(
    ...
    def test_missing_processor_dest_email_returns_400(self, client, auth_headers, draft_debate, app):
...
class TestEmailSubmissionDraftRateLimit:
    def test_rate_limit_decorator_configured_for_draft_endpoint(self, monkeypatch):
        ...
        try:
            ...
            draft_calls = [
                c for c in captured_calls if c["limit_value"] == "5 per hour"
            ]
