...
class DebateEngine:
    ...
    def __init__(self, fact_check_mode: str = "OFFLINE", enable_async_fact_check: bool = True):
        """
        ...
        self.extraction_engine = ExtractionEngine(fact_check_skill=self.fact_checker)
    ...
    def create_debate(
        self, motion: str, moderation_criteria: str, debate_frame: str, user_id: str | None = None
    ) -> Debate:
    ...
    def _apply_modulation(self, post: Post) -> Post:
        ...
        blocked_keywords = ["harass", "attack", "kill", "die", "stupid", "idiot"]
    ...
    def _extract_facts(self, post: Post, topic_id: str) -> list[CanonicalFact]:
        ...
        fact_spans, _ = self.extraction_engine.extract_spans_from_post(
            post.post_id, post.facts, post.inference, post.side.value, topic_id
        )

        # Extract facts with fact checking
        extracted_facts = self.extraction_engine.extract_facts_from_spans(
            fact_spans, topic_id, post.side.value, post.post_id
        )
    ...
    def _extract_arguments(self, post: Post, facts: list[CanonicalFact]) -> list[CanonicalArgument]:
        ...
        arg = CanonicalArgument(
            ...
            merged_provenance=[],
            reasoning_score=0.6
            + (0.2 if post.counter_arguments else 0),  # Bonus for addressing counters
            reasoning_iqr=0.15,
        )
    ...
    def generate_snapshot(self, debate_id: str, trigger_type: str = "activity") -> Snapshot:
        """
        ...
        self.scoring_engine.compute_counterfactuals(
            topics, topic_facts, topic_arguments, topic_content_mass
        )

        # Create snapshot
        snapshot_id = (
            f"snap_{datetime.now().strftime('%Y-%m-%dT%H%MZ')}_{len(debate.snapshots):04d}"
        )
