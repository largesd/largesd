...
def main():
    parser = argparse.ArgumentParser(description="Blind Debate Adjudicator Server v2")
    parser.add_argument(
        "--port", "-p", type=int, default=5000, help="Port to run the server on (default: 5000)"
    )
    parser.add_argument(
    ...
    if args.llm_provider in ["openrouter", "openrouter-multi"] and not os.getenv(
        "OPENROUTER_API_KEY"
    ):
    ...
    from backend.app_v2 import app, current_debate, debate_engine
