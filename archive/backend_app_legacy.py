...
# Initialize debate engine and database
...
def _debate_response_payload(debate):
    motion = debate.motion or debate.resolution
    return {
        "debate_id": debate.debate_id,
        "motion": motion,
        "resolution": motion,
        "moderation_criteria": debate.moderation_criteria,
        "debate_frame": debate.debate_frame,
        "scope": debate.scope,
...
def create_debate():
    """Create a new debate - requires authentication"""
    ...
    if not user_id:
        return jsonify(
            {"error": "Authentication required to create a debate", "code": "AUTH_REQUIRED"}
        ), 401
...
def get_debate():
    ...
    payload.update({"pending_posts": len(current_debate.pending_posts), "has_debate": True})
...
def submit_post():
    ...
    user_id = get_jwt_identity()
    if not user_id:
        return jsonify({"error": "Authentication required to post", "code": "AUTH_REQUIRED"}), 401
...
def get_audits():
    ...
    fact_overlap = {"p10": 0.72, "p50": 0.83, "p90": 0.91, "iqr": 0.10}

    arg_overlap = {"p10": 0.69, "p50": 0.81, "p90": 0.89, "iqr": 0.13}
    ...
    return jsonify(
        {
            ...
            "extraction_stability": {"fact_overlap": fact_overlap, "argument_overlap": arg_overlap},
