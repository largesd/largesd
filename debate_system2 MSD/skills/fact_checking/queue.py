...
from .models import FactCheckJob, FactCheckResult, FactCheckStatus
...
class FactCheckQueue:
    ...
    def __init__(self, max_size: int = 1000):
        ...
        self._processor: Optional[Callable[[FactCheckJob], FactCheckResult]] = None
    ...
    def get_job(self, job_id: str) -> Optional[FactCheckJob]:
        """Get job by ID"""
        with self._lock:
            return self._jobs.get(job_id)
    
    def get_result(self, job_id: str) -> Optional[FactCheckResult]:
    ...
    def get_pending_result(self, claim_hash: str, fact_mode: str, 
                          allowlist_version: str) -> Optional[FactCheckJob]:
        """
        ...
        with self._lock:
            for job in self._jobs.values():
                if (job.claim_hash == claim_hash and 
                    job.fact_mode == fact_mode and
                    job.allowlist_version == allowlist_version and
