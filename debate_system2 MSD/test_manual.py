...
def run_scenario_renewable_energy():
...
def run_modulation_test():
    ...
    for side, facts, inference, description in test_cases:
        print(f"\n[Test: {description}]")
        tester.submit_post(side=side, facts=facts, inference=inference)
    
    # Generate snapshot to see allowed posts
    print("\n[Generating snapshot to see allowed posts...]")
    tester.generate_snapshot()
...
def main():
    ...
    if command == "server-check":
        ...
        success = run_scenario_ai_regulation()
        sys.exit(0 if success else 1)
    
    elif command == "scenario-energy":
        success = run_scenario_renewable_energy()
        sys.exit(0 if success else 1)
    
    elif command == "modulation":
        success = run_modulation_test()
        sys.exit(0 if success else 1)
