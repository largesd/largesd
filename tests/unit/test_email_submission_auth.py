...
def test_reject_expired_token(config):
    ...
    token = generate_email_submission_token(config, {"type": "email_submission"}, now=past)
...
def test_token_issued_by_flask_endpoint_simulated_verified_standalone(config, base_submission):
