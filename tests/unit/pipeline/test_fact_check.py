...
def test_fact_check_stage_resolves_facts(engine):
    fact = ExtractedFact(fact_id="f1", fact_text="ft", topic_id="t1", side="FOR", fact_check_job_id="j1", fact_check_status="pending")
    ctx = PipelineContext(
...
def test_fact_check_stage_empty_extracted(engine):
    ctx = PipelineContext(
        debate_id="d1", job_id="j1", request_id="r1",
        trigger_type="manual", engine=engine, extracted={},
    )
    ctx = fact_check_stage(ctx)
