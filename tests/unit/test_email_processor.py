...
from backend.email_submission_parser import (
    EmailSubmission,
    EmailSubmissionError,
    EmailSubmissionParser,
...
def test_github_publish_retries_on_failure():
    ...
    for _ in range(max_retries):
        attempt += 1
        try:
            _result = mock_publisher.publish_json(payload={}, commit_message="test")
            success = True
            break
        except Exception:
...
def test_parse_for_processor_rejects_legacy_when_auth_required():
    ...
    result = parser.parse_for_processor(
        body, "test@example.com", "Test", require_auth=True, allow_legacy=False
    )
    assert result.decision == "reject"
...
def test_parse_for_processor_accepts_legacy_when_allowed():
    ...
    result = parser.parse_for_processor(
        body, "test@example.com", "Test", require_auth=True, allow_legacy=True
    )
    assert result.decision == "accept"
    assert result.version_hint == "v1"
...
def _make_mock_mail(
    body_text, from_addr="test@example.com", subject="Test Subject", to_addr="dest@example.com"
):
    msg = EmailMessage()
    msg["From"] = from_addr
    msg["To"] = to_addr
...
class TestEmailProcessorV3Auth:
    ...
    def processor(self):
        ...
        with (
            patch("backend.email_processor.DebateEngineV2"),
    ...
    def test_valid_signed_v3_calls_submit_post_with_user_id(self, processor):
        ...
        with (
            patch.object(
                processor.parser,
                "parse_for_processor",
                return_value=_make_accept_result(submission),
            ),
            patch(
    ...
    def test_edited_side_does_not_call_submit_post(self, processor):
        ...
        with (
            ...
            patch.object(processor, "_send_ack") as _mock_send_ack,
        ):
            processor._process_message(mock_mail, b"1")
            processor.debate_engine.submit_post.assert_not_called()
