...
class TestEmailSubmissionDraft:
    ...
    def test_logged_in_returns_v3_body_with_auth_token(self, client, auth_headers, draft_debate):
    ...
    def test_payload_hash_matches_body_and_recomputes(self, client, auth_headers, draft_debate):
        data = self._post_draft(client, auth_headers, draft_debate)
        body_hash_line = [
            line for line in data["body"].split("\n") if line.startswith("Payload-Hash:")
        ][0]
