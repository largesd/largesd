...
from backend.governance import AppealStatus, GovernanceManager, IncidentSeverity
...
def test_changelog():
    ...
    db = DebateDatabase(db_path=TEST_DB_PATH)
...
def test_appeals():
    ...
    print("✓ Appeal reviewed and accepted")
...
def test_fairness_audits():
    ...
    parity = gov.compute_demographic_parity(
        debate_id="debate_fairness_test", demographic_attribute="source_type", outcomes=outcomes
    )
...
def test_incidents():
    ...
    print("✓ Incident resolved with additive snapshot")
...
def test_governance_summary():
    ...
    print(
        f"✓ Appeals: {summary['appeals_summary']['total_appeals']} total, {summary['appeals_summary']['pending_count']} pending"
    )
